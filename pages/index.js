...
const itemlist = [
  ...
  },
  {
    ...
    city: "New Delhi",
    details:
      "Dolor dolor consectetur id aliquip laborum et. Consequat velit duis reprehenderit culpa aute aliqua laborum voluptate eiusmod. Aliquip et commodo nostrud et laborum cillum enim ullamco in enim irure qui.",
    details:
...
export default function Home() {
  ...
  const mintNFT = async (tokenId) => {
    ...
    try {
      ...
      // const tx = await TravelContract.mint(tokenId).send({
      //   gasLimit: 100000,
      // });
  ...
  const checkPosition = async (tokenId) => {
    ...
    const checkAdd = async (position) => {
      if (tokenId === items.length - 1) {
        toast(
          `💡 Fun Fact: Ethernals NFTs can be minted from anywhere in the world!`
        );
        ...
        try {
          ...
          if (
            add.address_components[0].long_name === items[tokenId].city ||
            state.address_components[0] == items[tokenId].city
          ) {
            // setLoading(true)
            await mintNFT(tokenId);
            // setLoading(false)
            ...
            toast(
              `We are detecting you at ${add.address_components[0].long_name}, visit ${items[tokenId].city} to mint the NFT!`
            );
            setLoading();
          }
        } catch (error) {
    ...
    <div>
      <div className={styles.navcont}>
        <Nav>
          {!walletConnected ? (
            ...
            <>
              <Link href="/profile">
                <a className={styles.abt}>Profile</a>
              </Link>
              <Button
      ...
      <div className={styles.container}>
        <div className={styles.hero}>
          ...
          <p>
            Let the quest to travel and exploring the world take a big leap into
            the another dimension with the support of NFTs and community.
            Showcase your achievements, exploration and get more closer with
            people who share similar interests and craziness all at one Place!{" "}
          </p>
        </div>
